...
export async function getHistory(historyType: 1 | 2, userId: string) {
  try {
    const response = await myAxios.get(
      `/muxlisaAI/user-history/${userId}/?history-type=${historyType}`,
    )
...
export async function clearHistory(historyType: 'textToVoice' | 'voiceToText', userId: string) {
  try {
    const response = await myAxios.delete(
      `/muxlisaAI/clear-chat-history/${userId}/?history-type=${historyType}`,
    )
...
export async function getCurrentAudio(
  currentAudioRef: RefObject<HTMLAudioElement>,
  audioId: string,
) {
  ...
  try {
    const response = await myAxios.get(
      `/muxlisaAI/current-audio/${audioId}/?audio-type=${audioType}`,
    )
