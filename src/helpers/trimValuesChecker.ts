...
const registerFields: string[] = ["name", "surname", "birthday", "email", "password", "confirm-password"];
  ...
  switch (type) {
    ...
    case "REGISTER":
        registerFields?.map((registerField: string) => {
            (formRef?.current?.[registerField]!).value = (formRef?.current?.[registerField]!).value?.trim();
        })
      break;
  }
}