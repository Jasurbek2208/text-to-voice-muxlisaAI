  ...
  return (
    <div className="relative w-max sm:my-5 my-2.5">
      <audio
        controls
        ref={currentAudioRef}
        className={`${isUser ? "self-end sm:mr-9 mr-3" : "self-start sm:ml-9 ml-3"} block max-w-[255px] p-3 text-gray-600 border border-gray-200 rounded-t-xl ${rounded} shadow outline-none hover:bg-gray-50 bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 duration-300`}
      >
        <source type="audio/*" />
        Brauzeringiz audio elementini qo'llab-quvvatlamaydi.
      </audio>
      {!audioSuccess && <div onClick={onAudioClick} className={`absolute top-0 ${isUser ? "self-end sm:rigth-9 rigth-3" : "self-start sm:left-9 left-3"} w-[255px] h-[76px] grid place-items-center rounded-t-xl ${rounded} bg-opacityWhite dark:bg-opacityDark duration-300`}>
        {
          !loading ? 
      ...
      <p className="absolute bottom-[2px] right-3 text-gray-600 dark:text-gray-300 text-xxs duration-300">{message?.date}</p>
    </div>
  );
}