...
import { changeLoading, textToVoiceHistoryAdd, textToVoiceHistoryChange } from '@store/store'
...
export default function TextToVoice() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        toast.error("Internetingiz o'chiq yoki texnik xato!", { position: 'top-center' })
      } finally {
  ...
  async function generateTextToVoice(data: ITextToVoiceHistory) {
    ...
    try {
      const { data: response } = await myAxios.post('/muxlisaAI/text-to-voice', formData, {
  ...
  return (
    <div className="max-h-dvh h-dvh overflow-hidden">
      ...
      <div
        ref={contentRef}
        className={`${
          textToVoiceHistory && textToVoiceHistory?.length > 0 && 'flex flex-col justify-end '
        } w-full max-h-messagesH h-full overflow-y-scroll scroll-no-width`}
      >
        <div
          className={`py-2.5 w-full max-w-[900px] mx-auto ${
            textToVoiceHistory?.length === 0 ? ' h-full max-h-min' : ' h-auto max-h-full'
          }`}
        >
          {textToVoiceHistory && textToVoiceHistory?.length > 0 ? (
            textToVoiceHistory?.map((message: ITextToVoiceHistory) => (
              <div key={message?._id} className="flex flex-col gap-0 w-full sm:my-5 my-2.5">
